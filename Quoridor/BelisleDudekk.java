...
* @version 4/16/13
*/
public class BelisleDudekk extends Player
{
     ...
     */
    public BelisleDudekk (int row, int col, int goal, String color)
    {
    ...
    public Coord move(Player opponent)
    {
        ...
        //printPath(route, row, col);
           ...
           if ((getPathLength(route) <= getPathLength(opponentRoute)) || (nextPossibleWall == null))          //compare shortest path of both
           {
   ...
   /**
    ...
    public Coord placeWall(Player opponent)
    {
        ...
        if (getPathRow(opponentRoute, 1) == getPathRow(opponentRoute, 2))  // depending on which we we travel
